...
from utils import save_pkl, load_pkl, inference_pytorch_instance_shuffle
...
#    if os.path.basename(args.ann_file_name).split('.')[0] != 'person':
#        return
    save_path = os.path.join(cfg.work_dir, '{}_concept_embedding.pkl'.format(os.path.basename(args.ann_file_name).split('.')[0]))
    if os.path.exists(save_path):
        print("{} exists".format(save_path))
        return
    # build the dataloader
    test_pipeline = cfg.data.test.pipeline
    test_pipeline[-2]['meta_keys'] = ['filename']
    ...
    returned_grads_dict = inference_pytorch_instance_shuffle(args, cfg, distributed, data_loader)
