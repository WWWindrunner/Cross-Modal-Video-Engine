...
def process_med_feats(feats_dict, model_type='transformer', num_crops=3):
    if model_type == 'transformer':
        for key in feats_dict.keys():
            feats_dict[key] = feats_dict[key][:, 0].view(-1, num_crops, feats_dict[key].size()[-1])
            feats_dict[key] = feats_dict[key].mean(1).detach().cpu()
            feats_dict[key] = feats_dict[key].mean(0)
            #feats_dict[key] = F.normalize(feats_dict[key], dim=0)
    else:
...
def single_gpu_test(model, data_loader, grad_getter, output_layers=[], model_type='transformer', as_tensor=True):  # noqa: F811
    ...
    for data in data_loader:
        ...
        grad = process_med_feats_new(grad, model_type=model_type)
        for layer in grad.keys():
            returned_grads_dict[layer].append(grad[layer])
        returned_grads_dict['label'].append(label)
        returned_grads_dict['pred_cls'].append(pred_cls.unsqueeze(0))
        # use the first key as main key to calculate the batch size
        batch_size = len(next(iter(data.values())))
        for _ in range(batch_size):
            prog_bar.update()
    for layer in returned_grads_dict.keys():
        returned_grads_dict[layer] = torch.stack(returned_grads_dict[layer], dim=0).squeeze()
...
def inference_pytorch(args, cfg, distributed, data_loader):
    ...
    if not distributed:
        ...
        returned_grads_dict = single_gpu_test(model, data_loader, grad_getter, output_layers=output_layers, model_type=args.model_type)
