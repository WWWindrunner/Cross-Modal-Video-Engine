...
def main():
    ...
    if args.launcher == 'none':
        ...
        distributed = True
        cfg.dist_params['timeout'] = datetime.timedelta(seconds=9000)
    ...
    ## 只使用backbone的部分特征
    #stop_layer = 'backbone/slow_path/layer4/2'
    #stop_layers = stop_layer.split('/')
    #partial_model = []
    #ch_model = model
    #for layer_name in stop_layers:
    #    for name, child in ch_model.named_children():
    #        print(name)
    #        if name == layer_name:
    #            ch_model = child
    #            break
    #        else:
    #            partial_model.append(child)
    #        
    #partial_model.append(ch_model)
    #partial_model.append(model.cls_head)
    #model = nn.Sequential(*list(partial_model))
