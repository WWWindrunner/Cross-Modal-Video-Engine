        ...
        if args.use_opencv:
            ...
            try:
                ...
                for i, vr_frame in enumerate(vr):
                    if i % args.ratio != 0:
                        #sample every ratio frames 防止数据量过大
                        frame_name += 1
                        continue
                    if vr_frame is not None:
                        ...
                        mmcv.imwrite(out_img,
                                        f'{out_full_path}/img_{frame_name + 1:05d}.jpg')
                        frame_name += 1
                    else:
                        warnings.warn(
                            'Length inconsistent!'
                            f'Early stop with {frame_name + 1} out of {len(vr)} frames.'
                        )
                        break
