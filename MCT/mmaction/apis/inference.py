...
from mmaction.core import OutputHook, InputHook
...
def init_recognizer(config, checkpoint=None, device='cuda:0', **kwargs):
    ...
    model = build_recognizer(config.model, test_cfg=config.get('val_cfg'))
...
def inference_recognizer(model, video, outputs=None, hook_type='output', as_tensor=True, max_len=6, return_scores=False, centercrop=False, shuffle_idx='without', **kwargs):
    ...
    Args:
        ...
        hook_type (str): output or input. Default: output.
    ...
    if centercrop:
        test_pipeline[4]['type'] = 'CenterCrop'
    if shuffle_idx != 'without':
        test_pipeline[-1]['shuffle_idx'] = shuffle_idx
    ...
    if input_flag == 'video':
        data = dict(filename=video, label=-1, start_index=0, modality='RGB', mask=mask)
    ...
    if hook_type == 'output':
        ...
        with InputHook(model, outputs=outputs, as_tensor=as_tensor) as h:
            with torch.no_grad():
                scores = model(return_loss=False, **data)[0]
            returned_features = h.layer_outputs if outputs else None
