...
def all_class_accuracy(scores, labels):
    """Calculate mean class accuracy.
    ...
    all_class_acc = np.array(
        [hit / cnt if cnt else 0.0 for cnt, hit in zip(cls_cnt, cls_hit)])

    return all_class_acc
...
def top_k_accuracy_shuffle(scores, labels, topk=(1, )):
    """Calculate top k accuracy score.
    ...
    bs = scores.size()[0]
    num_classes = scores.size()[1] // 2

    normal_scores = scores[:, :num_classes]
    shuffle_scores = scores[:, num_classes:]
    scores = torch.cat([normal_scores, shuffle_scores], dim=0).numpy()
    
    gt_labels = torch.zeros(bs).long()
    shuffle_labels = torch.ones(bs).long()
    labels = torch.cat([gt_labels, shuffle_labels], dim=0).tolist()
    labels = np.array(labels)[:, np.newaxis]
    for k in topk:
        max_k_preds = np.argsort(scores, axis=1)[:, -k:][:, ::-1]
        match_array = np.logical_or.reduce(max_k_preds == labels, axis=1)
...
def mean_average_precision(scores, labels):
    ...
    """
    results = []
    scores = torch.Tensor(np.stack(scores)) #[num_video, seq_len*num_classes]
    labels = torch.Tensor(np.stack(labels)) #[num_video, seq_len, num_classes]
    video_num, seq_len, num_classes = labels.size()
    scores = scores.view(video_num, seq_len, num_classes)
    print(scores.size(), labels.size())
    
    scores_for_eval = []
    labels_for_eval = []
    for video_scores, video_labels in zip(scores, labels):
        end_token_num = torch.sum(video_labels[:,-1]==1) #[seq_len]
        eval_token_num = video_scores.size()[0] - end_token_num + 1
        scores_for_eval.append(video_scores[:eval_token_num])
        labels_for_eval.append(video_labels[:eval_token_num])

    scores_for_eval = torch.cat(scores_for_eval, dim=0)
    labels_for_eval = torch.cat(labels_for_eval, dim=0)
    
    scores_for_eval = scores_for_eval.permute(1,0)
    labels_for_eval = labels_for_eval.permute(1,0)
    print(scores_for_eval.size(), labels_for_eval.size())
    
    for score, label in zip(scores_for_eval, labels_for_eval):
        precision, recall, _ = binary_precision_recall_curve(score.numpy(), label.numpy())
        ap = -np.sum(np.diff(recall) * np.array(precision)[:-1])
        results.append(ap)
...
def all_average_precision(scores, labels):
    """all average precision for multi-label recognition.
