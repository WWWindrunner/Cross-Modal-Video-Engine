...
class RandomShuffleFrames:
    ...
    Args:
        random_state (int): random shuffle seed.
    """

    def __init__(self, shuffle_idx):
    ...
    def __call__(self, results):
        ...
        Args:
            results (dict): The resulting dict to be modified and passed
                to the next transform in pipeline.
        """
        if self.shuffle_idx == 'adaptive':
            frame_num = results['imgs'].size()[2]
            shuffle_idx = list(range(frame_num))
            random.shuffle(shuffle_idx)
            shuffle_idx = torch.Tensor(shuffle_idx).int()
            results['shuffle_idx'] = shuffle_idx            
            results['imgs'] = results['imgs'].index_select(2, shuffle_idx)
        elif self.shuffle_idx == 'adaptive_test':
            frame_num = results['imgs'].size()[2]
            shuffle_idx = list(range(frame_num))
            random.shuffle(shuffle_idx)
            shuffle_idx = torch.Tensor(shuffle_idx).int()
            results['imgs'] = results['imgs'].index_select(2, shuffle_idx)
        elif self.shuffle_idx == 'training':
            frame_num = results['imgs'].size()[2]
            shuffle_idx = list(range(frame_num))
            random.shuffle(shuffle_idx)
            shuffle_idx = torch.Tensor(shuffle_idx).int()
            results['shuffle_imgs'] = results['imgs'].index_select(2, shuffle_idx)

        elif self.shuffle_idx is not None:
            results['imgs'] = results['imgs'].index_select(2, self.shuffle_idx)
