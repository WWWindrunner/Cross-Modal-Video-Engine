...
from ..core import (mean_average_precision, mean_class_accuracy, all_class_accuracy,
                    mmit_mean_average_precision, top_k_accuracy, top_k_accuracy_shuffle)
...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def evaluate(self,
        ...
        allowed_metrics = [
            'top_k_accuracy', 'mean_class_accuracy', 'all_class_accuracy', 'mean_average_precision',
            'mmit_mean_average_precision', 'top_k_accuracy_shuffle'
        ]
        ...
        for metric in metrics:
            ...
            if metric == 'top_k_accuracy':
                ...
                continue
            if metric == 'top_k_accuracy_shuffle':
                topk = metric_options.setdefault('top_k_accuracy_shuffle',
                                                 {}).setdefault(
                                                     'topk', (1, 5))
                if not isinstance(topk, (int, tuple)):
                    raise TypeError('topk must be int or tuple of int, '
                ...
                top_k_acc = top_k_accuracy_shuffle(results, gt_labels, topk)
                log_msg = []
                for k, acc in zip(topk, top_k_acc):
                    eval_results[f'top{k}_acc'] = acc
                    log_msg.append(f'\ntop{k}_acc\t{acc:.4f}')
