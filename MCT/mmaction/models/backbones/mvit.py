...
class MViT(nn.Module):
    ...
    def __init__(self, cfg_path, pretrained=None, no_grad=False):
        super().__init__()
        #args = parse_args()
        #args.cfg_files = [cfg_path]
        #cfg = load_config(args, args.cfg_files[0])
        cfg = load_config(None, cfg_path)
    ...
    @staticmethod
    def _change_keys(self, state_dict):
        new_dict = OrderedDict()
        for keys in state_dict:
            if 'projection' not in keys:
                new_dict['backbone.'+keys] = state_dict[keys]
            else:
                new_dict['cls_' + keys] = state_dict[keys]
        return new_dict
    @staticmethod
    def _init_weights(self, pretrained=None):
        ...
        if isinstance(self.pretrained, str):
            ...
            state_dict = self._change_keys(self, state_dict)
            print('*'*50)
            print(state_dict.keys())
