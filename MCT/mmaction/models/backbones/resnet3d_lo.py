...
class Bottleneck3d_lo(nn.Module):
    ...
    def forward(self, x, emb_list=[]):
        ...
        if self.non_local:
            out = self.non_local_block(out)
        emb_list.append(self.avg_pool(out))
        return out, emb_list


@BACKBONES.register_module()
class ResNet3d_lo(nn.Module):
    """ResNet 3d backbone.
    ...
    arch_settings = {
        ...
        501: (Bottleneck3d, (3,)),
        502: (Bottleneck3d, (3, 4)),
        503: (Bottleneck3d, (3, 4, 6)),
        ...
        if self.no_grad:
            with torch.no_grad():
                ...
                for i, layer_name in enumerate(self.res_layers):
                    ...
                    x, emb_list = res_layer(x, emb_list)
                    if i == 0 and self.with_pool2:
                        x = self.pool2(x)
                    if i in self.out_indices:
                        outs.append(x)
                if len(outs) == 1:
                    return outs[0], emb_list
        else:
            x = self.conv1(x)
            if self.with_pool1:
                x = self.maxpool(x)
