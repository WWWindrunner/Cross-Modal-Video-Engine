...
class BaseRecognizer(nn.Module, metaclass=ABCMeta):
    ...
    def __init__(self,
        ...
        elif backbone['type'].startswith('CLIP'):
            with open(backbone['model_path'], 'rb') as opened_file:
                state_dict = torch.jit.load(opened_file, map_location='cpu').state_dict()
                #self.backbone = torch.jit.load(opened_file)#, map_location=self.cls_head.device)
            backbone['state_dict'] = state_dict
    ...
    def init_weights(self):
        ...
        elif self.backbone_from in ['torchvision', 'timm', 'clip']:
    ...
    def average_clip(self, cls_score, num_segs=1, seq_gen=False):
        ...
        if not seq_gen:
            ...
            batch_size = cls_score.shape[0]
            seq_len = cls_score.shape[1]
            num_class = cls_score.shape[2]
            cls_score = cls_score.view(batch_size // num_segs, num_segs, seq_len, num_class)
        if average_clips == 'prob':
            if seq_gen:
                cls_score = cls_score.mean(dim=1)
            else:
                cls_score = F.softmax(cls_score, dim=-1).mean(dim=1)
