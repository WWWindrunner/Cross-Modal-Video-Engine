...
class Recognizer3D_CLIP(BaseRecognizer):
    ...
    def extract_feat(self, imgs):
        ...
        """
        batch_size, channels, frms, w, h = imgs.size()
        imgs = imgs.permute(0, 2, 1, 3, 4).reshape(-1, channels, w, h)
        with torch.no_grad():
            x = self.backbone.encode_image(imgs)
        x = x.reshape(batch_size, frms, -1).mean(1).float()
        return x