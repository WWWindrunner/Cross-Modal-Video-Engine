...
class GradCAM:
    ...
    def reshape_transform(tensor, height=7, width=7):
        ...
        return result
    def _calculate_localization_map(self, inputs, use_labels, delta=1e-20, is_transformer=False, num_frms=0, img_size=-1, patch_size=-1):
        ...
        if self.is_recognizer2d:
            ...
            if is_transformer:
                ...
                # for timesformer
                gradients = gradients[:, 1:, :].permute(0, 2, 1)
                gradients = gradients.reshape(gradients.size()[0], gradients.size()[1], num_frms, num_patches, num_patches)
                activations = activations[:, 1:, :].permute(0, 2, 1)
                activations = activations.reshape(activations.size()[0], activations.size()[1], num_frms, num_patches, num_patches)
                # for mvit
                #gradients = gradients[:, 1:, :].permute(0, 2, 1)
                #gradients = gradients.reshape(gradients.size()[0], gradients.size()[1], -1, 7, 7)
                #activations = activations[:, 1:, :].permute(0, 2, 1)
                #activations = activations.reshape(activations.size()[0], activations.size()[1], -1, 7, 7)
            # source shape: [B, C', Tg, H', W']
            #print(gradients.flatten())
            #print(torch.norm(gradients))
            #print(activations.flatten())
            print('gradients size:{}'.format(gradients.size()))
            print('activations size:{}'.format(activations.size()))
    ...
    def __call__(self, inputs, use_labels=False, alpha=0.5, is_transformer=False, num_frms=0, img_size=-1, patch_size=-1):
        ...
        localization_map, preds = self._calculate_localization_map(
            inputs, use_labels=use_labels, is_transformer=is_transformer, num_frms=num_frms, img_size=img_size, patch_size=patch_size)
