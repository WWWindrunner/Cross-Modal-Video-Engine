...
def combiner_training_fiq(train_dress_types: List[str], val_dress_types: List[str],
    ...
    training_path: Path = Path(
        base_path / f"models/combiner_trained_on_fiq_{clip_model_name}_{args.save_name}")
    training_path.makedirs(exist_ok=False, parents=True)
...
def combiner_training_composedvideo(projection_dim: int, hidden_dim: int, num_epochs: int, clip_model_name: str,
    ...
    training_start = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    training_path: Path = Path(
        base_path / f"models/version_10/{clip_model_name}/{args.save_name}")
    ...
    for epoch in range(num_epochs):
        ...
        with experiment.train():
            ...
            combiner.train()
            train_bar = tqdm(relative_train_loader, ncols=150)
            for idx, ((reference_feas, reference_middle_feas), (target_feas, target_middle_feas), captions) in enumerate(train_bar):  # Load a batch of triplets
                ...
                with torch.cuda.amp.autocast():
                    logits = combiner((reference_feas, reference_middle_feas), text_features, (target_feas, target_middle_feas))
                    ground_truth = torch.arange(images_in_batch, dtype=torch.long, device=device)
                    loss = crossentropy_criterion(logits, ground_truth)
                    # loss_2 = lossmetric(logits_2[0], logits_2[1])
                    # loss = loss+loss_2
...
if __name__ == '__main__':
    ...
    parser.add_argument("--combiner-lr", default=1e-3, type=float, help="Combiner learning rate")
    ...
    parser.add_argument("--save_name", default='cvpr_22_middle_trans_fc_kc_qt_vc_relu_dropout_lxf_output_fusion_finnal_with_middle_level_text_fea_k_v_p_r_transformer', action='store_true',
                        help="Save only the best model during training")
